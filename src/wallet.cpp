...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // Important: versions of talkcoin before 0.8.6 had a bug that inserted
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend,
                                ...
                                std::string chat_nick, std::string chat_message, std::string chat_data, int64 vote2)
{
    int64 nValue = 0;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-talkcoin-address
                                ...
                                std::string chat_nick, std::string chat_message, std::string chat_data, int64 vote2)
{
    ...
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, chat_nick, chat_message, chat_data, vote2);
}
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee, int64 vote2)
{
    ...
    string strError;
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, strError, NULL, "", "", "", vote2))
    {
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CTalkcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CTalkcoinAddress(address).ToString());
}
